...
plot_synth <- function(matrices, weights) {
  ...
  df |> 
    ggplot(aes(x = year, y = outcome, linetype = mode, pch = mode)) +
    geom_point() +
    geom_line() +
...
iwalk(plot_list, \(p, i) ggsave(
  plot = p + labs(title = i), 
  filename = paste0("img/synplots/trend_", i, ".png"), 
...
ggsave("img/balance_plot_smd.png", width = 7, height = 7, bg = "white")
...
# Full data plot
synth_control_df |> 
  ggplot(aes(x = year, y = outcome, colour = mode)) +
  ...
  labs(
    ...
    x = "Year", 
    y = "School performance (ISLED score)", 
    colour = "", fill = ""
  ) +
  ylim(25, 75) +
  theme(legend.position = "top", legend.justification = "right")

ggsave("img/synthetic_control_plot.png", width = 12, height = 7, bg = "white")
...
ggsave("img/synthetic_control_plot_data_dist.png", width = 12, height = 7, bg = "white")
...
ggsave("img/synthetic_control_plot_confidence.png", width = 12, height = 7, bg = "white")
...
ggplot(
  ...
  labs(
    ...
    # y = "",
    x = "ACE (improvement in ISLED score)"
  ) #+
  # theme(axis.text.y = element_blank())

ggsave("img/ace.png", width = 10, height = 4.5, bg = "white")
